...
public class ControleDeletarFornecedor extends ControleDeletar{

	private TelaConsultaFornecedor telaConsultaFornecedor;
	private ArrayList<FornecedorVO> fornecedores;
	
	public ControleDeletarFornecedor(TelaConsultaFornecedor tela) {
		this.telaConsultaFornecedor = tela;
	}

	@Override
	protected void deletar() {
		if (this.telaConsultaFornecedor.getTabela().getSelectedRow()==-1) {
			JOptionPane.showMessageDialog(this.telaConsultaFornecedor, "Por favor selecione uma linha.");
			
		}else {
			
			Fachada fachada =new Fachada();
			try {
				fachada.excluirFornecedor(Integer.parseInt(this.telaConsultaFornecedor.getTabela().getValueAt(this.telaConsultaFornecedor.getTabela().getSelectedRow(), 0).toString()),
						this.telaConsultaFornecedor.getTabela().getValueAt(this.telaConsultaFornecedor.getTabela().getSelectedRow(), 1).toString());
				
				JOptionPane.showMessageDialog(this.telaConsultaFornecedor, "Fornecedor excuido com sucesso.", "Informa"+(char)231+""+(char)227+"o", JOptionPane.INFORMATION_MESSAGE);
